...
app.use(cors({
  origin: function (origin, callback) {
    // 允許無 origin 的請求（同源 / server-to-server / 擴充套件）
    if (!origin) return callback(null, true);
    // 允許 Chrome 擴充套件
    if (origin.startsWith('chrome-extension://')) {
      if (ALLOWED_EXTENSION_IDS.length === 0) return callback(null, true); // 開發階段先放行所有擴充套件
      if (ALLOWED_EXTENSION_IDS.includes(origin)) return callback(null, true);
      return callback(new Error('Extension not authorized'));
    }
    // 允許白名單域名
...
function trackStats(req, res, next) {
  ...
  res.json = function(data) {
    const level = data?.result?.level || data?.newLevel;
    if (level) {
...
function setCachedResult(domain, level, label, riskScore = 0) {
  ...
  if (resultCache.size >= MAX_CACHE_SIZE) {
    const oldest = [...resultCache.entries()]
      ...
      .slice(0, 1000);
    oldest.forEach(([k]) => resultCache.delete(k));
  }
...
async function loadOpenPhish() {
  ...
  } catch (e) { console.log('⚠️ OpenPhish 失敗：', e.message); }
}
...
async function loadMajestic() {
  try {
    ...
    res.data.split('\n').slice(1, 50001).forEach(line => {
      const p = line.split(',');
      if (p[2]) majesticList.add(p[2].trim().toLowerCase());
    });
    console.log(`✅ Majestic Million 載入：${majesticList.size} 筆`);
  } catch (e) { console.log('⚠️ Majestic 失敗：', e.message); }
}
...
function loadNPABlacklist() {
  ...
  } catch (e) { console.log('⚠️ 165 黑名單失敗：', e.message); }
}
...
const TLD_TIER2_STRONG = new Set(['.xyz', '.top', '.work', '.click', '.loan', '.shop', '.store']);
...
function calcSoftRisk(factors) {
  ...
  // ── v2.3：相關性修正（無惡意信號衰減）──
  // 如果群5（信譽）和群6（行為）均無惡意信號，群1~4 的 risk 乘以衰減係數
  ...
  // v2.3：使用調整後的值計算，負值正常參與計算（讓信任加分降低風險）
...
// 手動校正 API（管理用，需要密碼）
app.post('/v1/thresholds/calibrate', async (req, res) => {
  const { key, adjustments } = req.body;
...
// V3.0 安全修正：內網 IP 黑名單（防 SSRF）
function isPrivateIP(hostname) {
  // 阻擋內網 IP、localhost、保留位址
...
app.post('/v1/check', async (req, res) => {
  ...
  try {
    ...
    return res.json({ status: 'ok', query: { domain, url_path: urlPath }, result });
  } catch {
...
app.post('/v1/deep-analyze', async (req, res) => {
  ...
  const shouldCrawl = !clusterHit && risk >= 0.20 && risk < 0.80 && 
                      crawler && process.env.CRAWLER_URL;
  
  if (shouldCrawl) {
    try {
      console.log(`[CRAWLER] 觸發爬蟲分析：${clean} (risk=${(risk*100).toFixed(1)}%)`);
  ...
  // ── 公開報告（只給前端看的白話文）──
  const publicReport = buildPublicReport(clean, checks, summary);
  ...
  return res.json({
    ...
    // v2.3：不再對外暴露 triggered_rules 和 trace
    // 內部審計透過 server log 的 [TRACE] 取得
  });
});
...
function buildPublicReport(domain, checks, summary) {
  // v2.3 原則：用戶只需要知道結論和建議，不需要知道偵測過程
  // 不暴露：API 名稱、分數、成功/失敗狀態、規則代號、IP、ASN、WHOIS 細節
  ...
  return `<strong>🔍 深度分析：${domain}</strong><br><br>` +
    `<span style="font-size:0.92rem;color:#374151;line-height:1.8">${detailSummary}</span>` +
...
app.post('/v1/l1/apply', async (req, res) => {
  const { domain, email, organization } = req.body;
  ...
  try {
    ...
    const existingL1 = await pool.query(
      'SELECT is_l1_verified, l1_expires FROM domains WHERE domain = $1',
      [clean]
    );
      ...
      if (age < 24 * 60 * 60 * 1000) {
        return res.json({
          ...
          instructions: [
            ...
            `2. 新增一筆 TXT 記錄`,
            `3. 名稱：${clean}（或 @）`,
            `4. 值：trustint-verify=${pendingToken.token}`,
            `5. 等待 DNS 生效後（通常 5-30 分鐘），點擊驗證按鈕`
          ]
        });
      }
    }

    // 檢查 DB 中是否有未過期的 pending token
    ...
    // 寫入 DB
    await pool.query(`
      INSERT INTO domains (domain, l1_token, l1_email, is_l1_verified, created_at, updated_at)
      VALUES ($1, $2, $3, false, NOW(), NOW())
      ON CONFLICT (domain) DO UPDATE SET 
        l1_token = $2, 
        l1_email = $3, 
        is_l1_verified = false,
        updated_at = NOW()
    `, [clean, token, email]);
    ...
    console.log(`[L1] 新申請：${clean} | Email: ${email}`);
...
app.post('/v1/l1/verify', async (req, res) => {
  ...
  try {
    ...
    if (dnsResult.found) {
      ...
      console.log(`[L1] ✅ 驗證成功：${clean}`);

      return res.json({
        status: 'verified',
        message: '🎉 恭喜！您的網域已通過 L1 官方驗證',
        domain: clean,
        level: 1,
        label: '官方已驗證',
        ...
        next_steps: [
          '您的網域現在會顯示為「L1 官方已驗證」',
          '驗證有效期為 1 年',
          '到期前 30 天會發送提醒至您的 Email',
          '您可以移除 DNS TXT 記錄（但建議保留）'
        ]
      });
    } else {
      // 驗證失敗
...
// API：列出所有 L1 網域（管理用）
app.get('/v1/l1/list', async (req, res) => {
  try {
    const result = await pool.query(`
